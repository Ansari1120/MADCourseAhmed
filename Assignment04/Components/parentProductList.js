...
const ParentProductList = () => {
  const { items: cart, updateItem: setCart } = useContext(MyContext);
  ...
  const handleAddtoCart = (item) => {
    const isItemInCart = cartItemExists(cart, item.id);
    if (isItemInCart) {
      setCart([...cart, item]);
    ...
    <SafeAreaView>
      <View style={styles.container}>
        ...
        <View style={{ marginBottom: 140 }}>
          ...
          {/* Assuming you want to display the quantity, not cart name */}
